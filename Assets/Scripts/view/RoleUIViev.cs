...
public class RoleUIViev : MonoBehaviour
{
    ...
    void Awake()
    {
        if (AvatarControl._instance == null)
            AvatarControl._instance = gameObject.GetComponent<AvatarControl>();
        DontDestroyOnLoad(AvatarControl._instance);
        AvatarControl._instance.skinnedSourceDict = new Dictionary<string, Dictionary<string, SkinnedMeshRenderer>>();
